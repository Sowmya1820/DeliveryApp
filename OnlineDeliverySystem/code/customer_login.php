...
// Check connection
    ...
    if (isset($_POST['UserID']) && isset($_POST['password'])) {
        ...
        $checkQuery = "SELECT * FROM Customer WHERE CustomerID = '$user_id' AND Password = '$password'";
        $result = $con->query($checkQuery);

        if ($result->num_rows > 0) {
            echo '<script>alert("Login Successful."); window.location.href="customer_home.html";</script>';
        } else {
