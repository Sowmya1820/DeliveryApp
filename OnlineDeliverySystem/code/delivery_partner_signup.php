...
//php for delivery partner signup
...
if (isset($_POST['user_id']) && isset($_POST['phone_number'])) {
    ...
    $checkQuery = "SELECT * FROM DeliveryPartner WHERE PartnerID = '$user_id'";
    ...
    if ($result->num_rows > 0) {
        ...
        if (strlen($phone_number) !== 10 || !is_numeric($phone_number)) {
            ...
            $sql = "INSERT INTO DeliveryPartner VALUES ('$user_id', '$full_name', '$password','$email', '$phone_number', '$availability_status')";
            if (mysqli_query($con, $sql)) {
                echo '<script>alert("Sign up successful :) , goahead and login"); window.location.href="delivery_partner_login.html";</script>';
            } else {
